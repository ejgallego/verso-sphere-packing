...
The following theorem is the key result of {citet ElkiesCohn}[]. Note that the
original theorem is stated for a class of functions more general then Schwartz
functions.
