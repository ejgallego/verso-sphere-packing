...
Of great interest to us is a specific family of functions, known as Schwartz
functions. The Fourier transform behaves particularly well when acting on
Schwartz functions. We elaborate in the following subsections.
...
```tex "lemma:Fourier-transform-is-automorphism" (slot := "proof")
\begin{proof}\leanok
...
Another reason we are interested in Schwartz functions is that they behave well
under infinite sums. This will be useful when proving the Cohn-Elkies linear
programming bound.
...
The decaying property of Schwartz functions means that they can be compared
with the absolutely convergent power series above.
...
```tex "lemma:Schwartz-summable" (slot := "proof")
\begin{proof}
...
We end with a crucial result on Schwartz functions, the statement of which
only makes sense because of the above result.
...
```tex "thm:Poisson-summation-formula" (slot := "proof")
\begin{proof}
...
While the Poisson summation formula over lattices can be stated in greater
generality and probably should be formalised as such in Mathlib due to its many
applications, we stick with Schwartz functions because that should be
...
```tex "thm:smooth-fast-decay-schwartz"
